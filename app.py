...
# Preset landmarks for fast, zero-delay routing
LANDMARKS = {
...
# Fetch Live Hubs from Supabase Cloud
def get_hubs():
...
# Atomic Real-Time Cloud Reservation
def create_reservation(hub_name):
    ...
    # Increment occupied slots in cloud database
        ...
        if st.sidebar.button("Calculate Route & Reserve Slot"):
            ...
            if not pass_id:
                ...
                with pass_col:
                    ...
                    st.success("Slot locked in Supabase Cloud for 30 minutes!")
        else:
            st.info("👈 Set your origin and parking hub in the sidebar, then click **Calculate Route & Reserve Slot**.")
    except Exception as e:
        st.error(f"Cloud Connection Error: {str(e)}. Please verify your Streamlit Secrets.")
    ...
    if st.button("Verify Pass at Gate"):
        if verify_id:
            ...
            if response.data:
                ...
                st.error("❌ INVALID PASS ID! Record not found in cloud database.")
        else:
